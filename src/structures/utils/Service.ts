import { CachedClock, Clock, Stopwatch, SystemClock } from "./Clock";
		...
		for (const task of tasks) {
			try {
				Promise.resolve(task()).catch((err) => {
					console.error({ err }, "unhandled error while executing async task");
				});
			} catch (err) {
				console.error({ err }, "unhandled error while executing task");
			}
		}
	}
